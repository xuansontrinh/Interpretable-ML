...
balanced_acc_measure_mean <- msr("classif.bacc", aggregator = mean)
...
xgboost_tune_ps <- ParamSet$new(list(
  ...
  ParamInt$new("nrounds", lower = 110, upper = 118), # maximum number of iterations
...
# 
# autoplot(bmr$filter(learner_ids = c(
#   "classif.log_reg", "classif.rpart", "classif.ranger.tuned", "classif.xgboost.tuned"
# )), measure=balanced_acc_measure) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
