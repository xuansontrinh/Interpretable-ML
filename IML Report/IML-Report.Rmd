...
After a few minutes of googling, he finds the very well known south german credit data set. As he doesn't have the bank's machine learning model, he fits his own model on the data set. His model performs really well on test data, which reassures him that it can be used in place of the bank's model. He also applies the model on him self, as he has been thinking of starting his own company and might need to take a loan from a bank to start off. The model predicts a _bad_ `credit_risk` for him. This motivates him to understand the model even more, perhaps with the aim of playing the system a bit. Now all that's left to do is for him to understand the model using IML methods and then look into his and his friends case.

In order to do so, he divides the problem into two. First, he wants to get a general understanding of the model and verify that its predictions make sense. Seconds, he would investigate, much like a detective, his and his friend's case (Bob's and Jame's feature instances are shown below).
```{r, include=FALSE}
library("iml")
```
