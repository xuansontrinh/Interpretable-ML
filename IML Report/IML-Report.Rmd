...
log_reg_pipeline_cat <- pos_cat %>>% log_reg_learner %>>% threshold
log_reg_glearner <- GraphLearner$new(log_reg_pipeline, id = "log_reg")
log_reg_glearner_cat <- GraphLearner$new(log_reg_pipeline_cat,
  id = "log_reg_cat"
)
...
log_reg_at_cat <- AutoTuner$new(
  learner = log_reg_glearner_cat,
  resampling = inner_cv5,
  measure = measure,
  search_space = ParamSet$new(list(ParamDbl$new("threshold.thresholds",
    lower = 0, upper = 1
...
rpart_pipeline_cat <- pos_cat %>>% rpart_learner %>>% threshold
rpart_glearner <- GraphLearner$new(rpart_pipeline, id = "rpart")
rpart_glearner_cat <- GraphLearner$new(rpart_pipeline_cat, id = "rpart_cat")
...
rpart_at_cat <- AutoTuner$new(
  learner = rpart_glearner_cat,
  resampling = inner_cv5,
  measure = measure,
  search_space = ParamSet$new(list(ParamDbl$new("threshold.thresholds",
    lower = 0, upper = 1
...
ranger_pipeline_cat <- pos_cat %>>% lrn("classif.ranger",
  predict_type = "prob"
) %>>% threshold
ranger_glearner <- GraphLearner$new(ranger_pipeline, id = "ranger")
ranger_glearner_cat <- GraphLearner$new(ranger_pipeline_cat, id = "ranger_cat")
...
ranger_at_cat <- AutoTuner$new(
  learner = ranger_glearner_cat,
  resampling = inner_cv5,
  measure = measure,
  search_space = ranger_tune_ps,
  terminator = terminator,
...
```{r}
# bmr_cat$aggregate(measure)
# autoplot(bmr_cat, measure = msr("classif.ce")) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
# autoplot(bmr_cat, measure = msr("classif.fbeta")) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
# autoplot(bmr_cat, measure = measure) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
