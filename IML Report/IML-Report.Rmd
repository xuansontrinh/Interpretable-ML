...
```
The benchmark shows that the svm model with `radial` kernel is most performant on average as shown in Figure 1. This model is a black box model.
...
```
```{r results, out.width='60%', fig.align='center', fig.cap='Evaluation', echo=FALSE}
knitr::include_graphics("plots/fbeta.jpeg")
```

We also observe that the performace of tree based models doesn't change significantly, when categorical variables are not coverted to numerics (See box plots in Figure labeled `fct_`).
