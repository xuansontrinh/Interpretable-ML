...
```{r out.width='50%', fig.align='center', fig.cap="\\label{fig:pipeops}Radial SVM pipeline graph", echo=FALSE}
...
$$
\hat f_{x_S}(x_S) = E_{x_C}[\hat f(x_S, x_C)] = \int \hat f(x_S, x_C)d\mathbb{P}(x_C) \approx \sum_{i = 1}^n \hat f (x_S, x_C^{(i)})
$$
where $\hat f$ is the model, $x_S$ are the features to get partial dependence function plotted and $x_C$ are the other features, $x_C^{(i)}$ are the actual feature values from the dataset, $n$ is the sample size. The approximation for the partial dependence function is calculated using the averages from the training data @molnar2019.\
...
**Permutation Feature Importance:** It measures the increase in prediction error after permuting the feature values. The feature importance for a feature $j$ given a dataset $X$ and a dataset $X^{perm}$ where the feature is permuted, is calculated as shown below:

$$
FI^{j} = \dfrac{e^{perm}}{e^{orig}}
$$
$$
e^{perm} = 1 - F_{1}(y, f(X^{perm}))
$$
$$
e^{orig} = 1 - F_{1}(y, f(X))
$$
