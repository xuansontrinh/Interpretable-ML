...
|Is being older mean better for `credit_risk`?|ALE|
...
|How can James get a `good` credit risk ?|Counterfactual Explanations|
|Why did Bob get a `bad` credit risk and how can he change it ?|Counterfactual Explanations|
...
3. Having no `property` should imply a bad `credit_risk`\
NaN


4. Is being older better for `credit_risk`?\
...
5. Is there an interaction between `job` and `credit_history`? People with delayed `credit_history` often have low quality `job`\
Figure \@ref(fig:job-interaction) shows the 2-way interactions between job and the other features. The interaction strength between `job` and `credit_history` is pretty weak. In general all the 2-way interactions are pretty weak.

```{r job-interaction, out.width='60%', fig.align='center', fig.cap="2-way interaction strength between `jobs' and  the other features.", echo=FALSE}
knitr::include_graphics("plots/h-4.png")
```
...
7. Being a foreign worker does not affect your credit risk. If this feature is important for prediction, this implies discrimination.\
...
8. The better the savings, the better your credit risk\
...
Figure \@ref(fig:shapley-james) shows that the most influential feature affecting jame's good prediction is his checking account's `status` compared to the average prediction for the data set.

```{r, include=FALSE}
shapley <- Shapley$new(model, x.interest = james)
```
```{r shapley-james, out.width='80%', fig.align='center', fig.cap="Shapley contributions to Jame's predictions", echo=FALSE}
plot(shapley)
```
...
```
4. How can James get a `good` credit risk ? \
NaN

5. Why did Bob get a `bad` credit risk and how can he change it ? \
NaN
