...
## Story Time
Bob is a 30 year old, data scientist in Munich. He has been married for 5 years with no kids. He recently got interested in Interpretable machine learning and works on it in his spare time.
Bob likes learning by doing, and so he looks very hard for an interesting problem to investigate. Suddenly, he recalls a conversation with his best friend, James, who wanted to purchase a car but was surprisingly denied a credit from the bank to do so. James is a Doctor, the same age as bob, but he is single. In order to help his friend understand the bank's decision, he decides to investigate the problem, but for that he needs a data set.
After a few minutes of googling, he finds the very well known south german credit data set. As he doesn't have the bank's machine learning model, he fits his own model on the data set. His model performs really well on test data, which reassures him that it can be used in place of the bank's model. He also applies the model on his self, as he has been thinking about moving out of their current appartment and buying a house soon for him and his wife. The model predicts a _bad_ `credit_risk`. Now all that's left to do is for him to understand the model using IML methods and then look into his and his friends case. 

In order to do so, he divides the problem in to two. First, he wants to get a general understanding of the model and verify that its predictions make sense. Seconds, he would investigate, much like a detective, his and his friend's case.
...
|What are the interaction effects between the features |H-Statistics|
...
|Because Bob is married and James isn't, Bob is interested in whether his material status has a positive or negative effect on the model's prediction|LIME|
...
The figure below shows the 2-way interactions between job and the other features. The interaction strength between `job` and `credit_history` is pretty weak. In general all the 2-way interactions are pretty weak.
...
8. Being a foreign worker does not affect your credit risk. If this feature is important for prediction, this implies discrimination.\
...
eff$plot()
```
```{r ale-foreign-worker, out.width='80%', fig.align='center', fig.cap="ALE Plot for `foreign-worker`", echo=FALSE}
...
ggplot(data = data, mapping = aes(x = foreign_worker, fill = credit_risk)) +
  geom_bar()
```
```{r geom-bar-foreign-worker, out.width='80%', fig.align='center', fig.cap="Bar Plot for `foreign-worker` with distribution of `credit-risk` in each level", echo=FALSE}
knitr::include_graphics("plots/geom_bar_foreign_worker.jpg")
```

9. The better the savings, the better your credit risk\
