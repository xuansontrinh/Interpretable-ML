...
## Story Time
Bob is a 30 year old, senior data scientist in Munich. He has been married for 5 years with no kids. He recently got interested in Interpretable machine learning and works on it in his spare time.
Bob likes learning by doing, and so he looks very hard for an interesting problem to investigate. Suddenly, he recalls a conversation with his best friend, James, who wanted to purchase a car and got a credit from the bank to do so. James is a doctor, the same age as Bob, but he is single. This makes Bob curious about credit risk predicitions. So he decides to investigate the problem, but for that he needs a data set.
After a few minutes of googling, he finds the very well known south german credit data set. As he doesn't have the bank's machine learning model, he fits his own model on the data set. His model performs really well on test data, which reassures him that it can be used in place of the bank's model. He also applies the model on himself, as he has been thinking of starting his own company and might need to take a loan from a bank to start off. The model predicts a _bad_ `credit_risk` for him. This motivates him to understand the model even more, perhaps with the aim of playing the system a bit. Now all that's left to do is for him to understand the model using IML methods and then look into his and his friend's case.

In order to do so, he divides the problem into two. First, he wants to get a general understanding of the model and verifies that its predictions make sense. Second, he would investigate, much like a detective, his and his friend's case (Bob's and Jame's feature instances are shown below).
```{r, include=FALSE}
library("iml")
```
...
|What can Bob do to improve his credit, i.e. to be considered `good` ?|Counterfactual Explanations|
...
The importance of features can be uncovered using Permutation Feature Importance (Figure \@ref(fig:feature-importance)). Through the increase in the prediction error, we can see that the most important feature for predicting `credit_risk` is `status` of your checking account, which stands out from the other features of the dataset. It is followed by `duration`, `amount`, `property`, `installment_rate`, etc. with gradual decrease in the importance.

```{r feature-importance, out.width='55%', fig.align='center', fig.cap="German Credit Feature Importance", echo=FALSE}
...
```{r interaction, out.width='60%', fig.align='center', fig.cap="Interaction Strength (H-Statistic) between each feature and all other features", echo=FALSE}
...
```{r pdp-ice-age, out.width='50%', fig.align='center', fig.cap="Uncentered PDP + ICE Plot of `age'", echo=FALSE}
knitr::include_graphics("plots/pdp_ice_age.jpg")
```

```{r pdp-ice-center-age, out.width='50%', fig.align='center', fig.cap="Centered PDP + ICE Plot of `age'", echo=FALSE}
knitr::include_graphics("plots/pdp_ice_centered_age.jpg")
```

```{r ale-age, out.width='50%', fig.align='center', fig.cap="ALE Plot of `age'", echo=FALSE}
...
Figure \@ref(fig:job-interaction) shows the 2-way interactions between job and the other features. The interaction strength between `job` and `credit_history` is pretty weak. In general all the 2-way interactions betweek `job` and other features are pretty weak.

```{r job-interaction, out.width='60%', fig.align='center', fig.cap="2-way interaction strength between `job` and  the other features.", echo=FALSE}
...
```{r ale-foreign-worker, out.width='50%', fig.align='center', fig.cap="ALE Plot for `foreign-worker'", echo=FALSE}
...
ggplot(data = data, mapping = aes(x = foreign_worker, fill = credit_risk)) +
  geom_bar()
```
```{r geom-bar-foreign-worker, out.width='50%', fig.align='center', fig.cap="Bar Plot for `foreign-worker` with distribution of `credit-risk' in each level", echo=FALSE}
...
```
```{r shapley-james, out.width='80%', fig.align='center', fig.cap="Shapley contributions to James's predictions", echo=FALSE}
...
max(data$amount)
```
```{r shapley-bob, out.width='80%', fig.align='center', fig.cap="Shapley contributions to Bob's predictions", echo=FALSE}
...
```{r counterfactuals-surface-bob, out.width='80%', fig.align='center', fig.cap="Counterfactuals surface plot of two features `duration` and `amount` of Bob's case", echo=FALSE}
