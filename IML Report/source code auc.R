...
log_reg_glearner <- GraphLearner$new(log_reg_pipeline, id = "log_reg", predict_type = "prob")
# log_reg_glearner$predict_type = "prob"



rpart_learner <- po(lrn("classif.rpart", predict_type="prob"))
rpart_pipeline <- pos %>>% rpart_learner
rpart_glearner <- GraphLearner$new(rpart_pipeline, id = "rpart", predict_type = "prob")
# rpart_glearner$predict_type = "prob"

fct_rpart_pipeline <- int_to_num %>>% scaler %>>% po_over %>>% rpart_learner
fct_rpart_glearner <- GraphLearner$new(fct_rpart_pipeline, id = 'fct_rpart', predict_type = "prob")
# fct_rpart_glearner$predict_type = "prob"





ranger_pipeline <- pos %>>% po(lrn("classif.ranger", predict_type="prob"))
ranger_glearner <- GraphLearner$new(ranger_pipeline, id='ranger', predict_type = "prob")
# ranger_glearner$predict_type = "prob"
...
fct_ranger_pipeline <- int_to_num %>>% scaler %>>% po_over %>>% po(lrn("classif.ranger", predict_type="prob"))
fct_ranger_glearner <- GraphLearner$new(fct_ranger_pipeline, id='fct_ranger', predict_type = "prob")
...
# Learners
linear_svm_glearner <- GraphLearner$new(linear_svm_pipeline, id = "linear_svm", predict_type = "prob")
poly_svm_glearner <- GraphLearner$new(poly_svm_pipeline, id = "poly_svm", predict_type = "prob")
radial_svm_glearner <- GraphLearner$new(radial_svm_pipeline, id = "radial_svm", predict_type = "prob")
...
autoplot(bmr, measure = msr("classif.auc")) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
autoplot(bmr, measure = msr("classif.bbrier")) + theme(axis.text.x = element_text(angle = 45, hjust = 1))